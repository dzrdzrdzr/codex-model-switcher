...
mkdir -p "$HOME/.codex" "$HOME/.codex-vscode-deepseek" \
...
#!/usr/bin/env bash
if [[ "${1:-}" == "--version" ]]; then echo "moonbridge-wrapper 0.1.0"; exit 0; fi
export CODEX_HOME="$HOME/.codex"
exec "$(dirname "$0")/codex.real" "$@"
MOCK
chmod 700 "$HOME/.vscode-server/extensions/openai.chatgpt-99.0.0/bin/linux-x86_64/codex"
bash "$PROJECT_ROOT/scripts/remote-helper.sh" install gpt high deepseek-v4-pro > /dev/null
...
CONFIG_SOURCE="$TEST_ROOT/test.config.toml"
MODELS_SOURCE="$TEST_ROOT/test.models.json"
...
bash "$PROJECT_ROOT/scripts/remote-helper.sh" install deepseek high deepseek-v4-pro "$CONFIG_SOURCE" "$MODELS_SOURCE" > /dev/null
...
! grep -q 'test-only-api-key' <<< "$STATUS"
...
test -n "$(find "$HOME/.codex-vscode-deepseek" -name 'auth.json.before-direct.*.bak' -print -quit)"
...
bash "$PROJECT_ROOT/scripts/remote-helper.sh" install gpt high deepseek-v4-pro > /dev/null
grep -q '^model = "gpt-test"$' "$HOME/.codex/config.toml"
echo 'PASS remote helper round trip (mock Codex, no live API)'
